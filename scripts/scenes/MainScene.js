...
export default class MainScene extends Phaser.Scene {
	...
	addBanner() {
		...
		Methods.addText(this, Constants.SCREEN_WIDTH / 2, 128, "Red Racer", "40px")
		...
		Methods.addText(this, Constants.SCREEN_WIDTH / 2, 176, "Car game", "24px")
			.setDepth(Constants.SCREEN_HEIGHT)
			.setOrigin(0.5)
			.setScrollFactor(0)
			.setColor(color)
	...
	addHighScore() {
		...
		try {
			if (Constants.IS_DEBUG === true) {
				localStorage.setItem("CarsHighScore", 0);
			}

			const height = Constants.SCREEN_HEIGHT;
			const highScore = localStorage.getItem("CarsHighScore");
